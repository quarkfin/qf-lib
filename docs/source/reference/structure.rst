...
The components included in this catalogue are used to Analyse strategy
...
- **data_providers** - market data adapters (CSV, Bloomberg, Quandl, and others) sharing a common ``DataProvider`` interface. Inside a backtest, the session exposes ``ts.data_provider``, which respects the simulation clock so that data “from the future” is not visible to strategies.
...
- **signals**: 
    ...
    * **signals_register** - components used to save signals processed by the ``PositionSizer``. The allow to later Analyse all signals generated by the model and to present them in a readable form. 
- **strategies**: 

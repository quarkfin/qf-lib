...
- ``EndTradingEvent``- occurs when the trading should stop (e.g. when the backtest should be terminated). It is published when the backtest runs out of data (via ``TimeFlowController``). In the future, this could also be triggered by the user (from GUI or console),
...
``RegularTimeEvents``. The only difference is that, they may occur with a
...
To avoid Saturdays and Sundays being generated by the ``next_trigger_time()`` you can
call exclude_weekends method on the ``PeriodicEvent`` class.

The ``PeriodicEvent`` is implemented only for frequencies DAILY and higher
(like MIN_60 or MIN_15). If you want to create ``Event`` for every month or quarter
we suggest you to use the ``RegularTimeEvents``.

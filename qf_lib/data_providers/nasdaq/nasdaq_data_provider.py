...
from qf_lib.common.tickers.tickers import NasdaqTicker, Ticker
...
try:
    ...
    is_nasdaq_installed = True
except ImportError:
    is_nasdaq_installed = False
    warnings.warn("No nasdaq installed. If you would like to use NasdaqDataProvider first install the quandl library.")


class NasdaqDataProvider(DataProvider):
    """
    Class providing the Nasdaq data.
    ...
    def __init__(self, settings: Settings):
        ...
        try:
            self.key = settings.nasdaq_key
            nasdaqdatalink.ApiConfig.api_key = self.key
        except AttributeError:
            self.logger.warning("No nasdaq_key parameter found in Settings. If you want to use NasdaqDataProvider, add "
                                "nasdaq_key in the settings json file.")

    def get_price(self, tickers: Union[NasdaqTicker, Sequence[NasdaqTicker]],
    ...
    def get_history(
            self, tickers: Union[NasdaqTicker, Sequence[NasdaqTicker]], fields: Union[None, str, Sequence[str]] = None,
    ...
    def _get_history(
            self, convert_to_prices_types: bool, tickers: Union[NasdaqTicker, Sequence[NasdaqTicker]],
            fields: Union[None, str, Sequence[str], PriceField, Sequence[PriceField]] = None,
            start_date: datetime = None, end_date: datetime = None) -> \
            Union[QFSeries, QFDataFrame, QFDataArray]:
        """
        NOTE: Only use one Nasdaq Database at the time.
        ...
        """
        tickers, got_single_ticker = convert_to_list(tickers, NasdaqTicker)
    ...
    def _get_result_for_single_database(self, convert_to_prices_types, ticker_group, fields, start_date, end_date):
        first_ticker = ticker_group[0]  # type: NasdaqTicker
    ...
    def _map_fields_to_str(self, fields: Sequence[PriceField], database_name: str, database_type: NasdaqDBType):
    ...
    def _str_to_price_field_map(self, database_name: str, database_type: NasdaqDBType):
        field_to_str = self._price_field_to_str_map(database_name, database_type)
        str_to_field = {field_str: field for field, field_str in field_to_str.items()}

        return str_to_field

    def _price_field_to_str_map(self, database_name: str, database_type: NasdaqDBType) -> Dict[PriceField, str]:
        if database_type == NasdaqDBType.Table and database_name == 'WIKI/PRICES':
            price_field_dict = {
            ...
            raise LookupError(
                "Nasdaq Database: {} is not supported. PriceField -> string mapping is required.".format(database_name)
            )
        return price_field_dict

    def _get_history_from_table(self, tickers_of_single_db: Sequence[NasdaqTicker], fields: Sequence[str],
                                start_date: datetime, end_date: datetime) -> Dict[NasdaqTicker, pd.DataFrame]:
        ...
        df = nasdaqdatalink.get_table(db_name, ticker=tickers_str, paginate=True, **field_options)
        ...
        for ticker_str, ticker_df in ticker_grouping:
            ticker = NasdaqTicker(ticker=ticker_str, database_name=db_name, database_type=NasdaqDBType.Table)
    ...
    def _get_history_from_timeseries(
            self, tickers: Sequence[NasdaqTicker], fields: Sequence[str], start_date: datetime, end_date: datetime):
        """
        NOTE: Only use one Nasdaq Database at the time. Do not mix multiple databases.
        """
        ...
        data = nasdaqdatalink.get(list(tickers_map.keys()), **kwargs)  # type: pd.DataFrame
        ...
        ticker_grouping = data.T.groupby(extract_ticker_name)
        ...
        for ticker, ticker_data_df in ticker_grouping:
            tickers_and_fields = (column_name.split(' - ') for column_name in ticker_data_df.T.columns)
            field_names = [field for (ticker, field) in tickers_and_fields]
            ticker_data_df.index = field_names
            ...
            ticker_to_df[ticker] = ticker_data_df.T
    ...
    def _select_only_required_fields(self, ticker, ticker_data, fields):
        ...
        if missing_fields:
            ...
            self.logger.warning("Columns {} have not been found in the Nasdaq response".format(missing_columns))
        ...
        if not fields_to_select:
            ...
            result = ticker_data.loc[:, list(fields_to_select)]
    ...
    def get_futures_chain_tickers(self, tickers: Union[FutureTicker, Sequence[FutureTicker]],
        ...
        raise NotImplementedError("Downloading Future Chain Tickers in NasdaqDataProvider is not supported yet")

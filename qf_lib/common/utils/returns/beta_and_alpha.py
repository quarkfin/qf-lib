...
def beta_and_alpha_full_stats(
        strategy_tms: QFSeries, benchmark_tms: QFSeries) -> Tuple[float, float, float]:
    """
    ...
    correlation_matrix = np.corrcoef(benchmark_returns, strategy_returns)
    r_value = correlation_matrix[0, 1]

    return beta, alpha, r_value

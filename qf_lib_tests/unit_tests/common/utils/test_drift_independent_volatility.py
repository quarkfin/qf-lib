...
class TestDriftIndependentVolatilityUtils(TestCase):
    def setUp(self):
        ...
        o = [100.55, 101.20, 103.29, 99.64, 126.93, 127.38, 125.39, 128.04, 124.17, 122.72, 123.83, 126.74]
        h = [105.16, 105.36, 104.13, 106.35, 130.47, 132.54, 128.83, 131.73, 128.29, 127.16, 126.27, 132.98]
        l = [98.07, 99.43, 100.03, 98.89, 115.37, 122.72, 120.48, 126.39, 121.27, 120.74, 122.01, 126.26]
        c = [101.20, 105.15, 101.12, 104.37, 121.72, 124.73, 125.29, 126.67, 122.92, 124.05, 122.20, 128.19]

        open = PricesSeries(data=o, index=tms, name=PriceField.Open)
        high = PricesSeries(data=h, index=tms, name=PriceField.High)
        low = PricesSeries(data=l, index=tms, name=PriceField.Low)
        close = PricesSeries(data=c, index=tms, name=PriceField.Close)
